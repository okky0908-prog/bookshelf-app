...
|---|---|---|---|---|
| id | bigint | × | 自動採番 | 主キー |
| name | varchar(30) | × | | タグ名。一意 |

- タグは全本棚で共通とする（要件定義書5.7）
- `name` の照合順序は `utf8mb4_bin` とし、文字列が完全に一致するときだけ同じタグとみなす（「Ruby」と「ruby」、「ミステリー」と「みすてりー」は別のタグになる）
- 前後の空白は保存前にアプリ側で取り除く
...
| tags | name | 一意 | 同じ名前のタグを重複して作らない（要件定義書5.7） |
...
| タグ名の同一判定 | 完全一致（utf8mb4_bin） | 大文字・小文字やひらがな・カタカナを区別しない照合順序 | MySQL 8.0 の標準の照合順序では、区別しない文字の組み合わせが直感とずれることがある。判定のルールを「完全一致」とはっきりさせるほうがわかりやすい |
