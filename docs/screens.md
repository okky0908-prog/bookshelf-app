...
- 本書の内容を操作できる形にしたモックアップ：https://claude.ai/artifact/W8iCFgdTJ9WnPUvgfjAAR1 （非公開。オーナーのみ閲覧可）
- 2026-09-26 に表示・操作を確認し、本書の内容で問題ないことを確認した
- 収録している画面：SCR-01（通常／グラフ表示＋タグ絞り込み中）、MDL-01（編集モード）、DLG-01、MDL-02（設定モード）
- モックアップで省略している点：書影画像の読み込み（色のブロックで代用）、グラフ開閉状態の保存、Escキーで閉じる操作、読み込み中・保存失敗の表示。また、タグの候補表示はDB設計で決めた表記ゆれの扱い（[database.md](database.md) 3.3.1）より前の仕様のままになっている
...
| C. ツールバー | キーワード入力欄、タグのセレクトボックス（「すべて」＋全タグ）、［クリア］、［＋書籍を追加］ | 入力すると自動で絞り込む（キーワードは入力が止まってから約300ms後。[api.md](api.md) API-05）。［クリア］で両方の条件を解除する。［＋書籍を追加］でMDL-01（登録モード）を開く |
